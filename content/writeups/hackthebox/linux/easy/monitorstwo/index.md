...
I've run into this same family of issues on several other boxes, which is worth noting for anyone building intuition across a range of targets rather than memorizing one machine at a time. On the Cacti side, [Monitored](/writeups/hackthebox/linux/medium/monitored/) pairs SNMP with a different Cacti CVE, and [Cactus](/writeups/tryhackme/linux/easy/cactus/) hits Cacti again from another angle. On the container-escape side, [Analytics](/writeups/hackthebox/linux/easy/analytics/) leaks its way out through environment variables, and [Jupiter](/writeups/hackthebox/linux/medium/jupiter/) does something similar. And for "crack a hash pulled straight from the application's own database," I'd point to [Cat](/writeups/hackthebox/linux/medium/cat/) and [Bizness](/writeups/hackthebox/linux/easy/bizness/) as close cousins of what I did here with `user_auth`.
